...
static void logToJournalIfRunAsService() {
  ...
  ** as described in https://systemd.io/JOURNAL_NATIVE_PROTOCOL */
  ...
  if (journal_stream != nullptr) {
    ...
    if (result.ec != std::errc{}) {
      spdlog::warn("malformed JOURNAL_STREAM (\"{}\"): {}, logging to console",
          journal_stream,
    ...
    if (fstat(STDERR_FILENO, &f_stderr) != 0) {
      spdlog::warn("unable to check stderr device and inode numbers: {}",
                   strerror(errno));
    } else if (device == f_stderr.st_dev && inode == f_stderr.st_ino) {
      auto journald = spdlog::systemd_logger_st("native_journal", "waybar",
                                                false);
